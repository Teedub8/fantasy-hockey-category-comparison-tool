...
def build_full_stats_df():
    """Build a full DataFrame of all players with stats"""
...
def highlight_comparison(row):
    """Highlight higher stat (green) and higher percentile (blue)"""
    colors = []
...
if st.button("Fetch Live NHL Stats"):
    with st.spinner("Fetching NHL player data... this may take 30-60 seconds"):
        try:
    ...
    st.info("Click 'Fetch Live NHL Stats' to load player stats.")
    df = pd.DataFrame()
